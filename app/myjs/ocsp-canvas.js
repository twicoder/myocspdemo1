ocspspace.Canvas = (function () {
  ...
  var initCanvas = function () {
    ...
    svg = d3.select('#canvas-container').append('svg')
      .on('contextmenu', function () {
        ...
        ocspspace.CanvasUtils.getSelection().classed('selected', false);

        // show the context menu on the canvas
        ocspspace.ContextMenu.show();
    ...
    var processGroupStatsBackground = defs.append('linearGradient')
      .attr('id','process-group-stats-background')
      .attr('x1','0%')
      .attr('y1','100%')
      .attr('x2','0%')
      .attr('y2','0%');
    ...
    var processorStatsBackground = defs.append('linearGradient')
      .attr('id','processor-stats-background')
      .attr('x1','0%')
      .attr('y1','100%')
      .attr('x2','0%')
      .attr('y2','0%');
    ...
    expirationBackground.append('stop')
      .attr('offset','0%')
      .attr('stop-color','#aeafb1');

    expirationBackground.append('stop')
      .attr('offset','100%')
      .attr('stop-color','#87888a');
      ...
      if (d3.event.shiftKey) {
        ...
        canvas.append('rect')
          ...
          .attr('stroke-width', function () {
            return 1 / ocspspace.Canvas.View.scale();
          })
          .attr('stroke-dasharray', function () {
            return 4 / ocspspace.Canvas.View.scale();
          })
          .datum(position);
    ...
    $(window).on('resize', function () {
      ...
      if (evt.ctrlKey || evt.metaKey) {
        if (evt.keyCode === 82) {
          // ctrl-r
          ocspspace.Actions.reloadStatus();

          evt.preventDefault();
        } else if (evt.keyCode === 65) {
          // ctrl-a
          ocspspace.Actions.selectAll();
          ocspspace.CanvasToolbar.refresh();

          evt.preventDefault();
        } else if (evt.keyCode === 67) {
          // ctrl-c
          ocspspace.Actions.copy(ocspspace.CanvasUtils.getSelection());

          evt.preventDefault();
        } else if (evt.keyCode === 86) {
          // ctrl-p
          ocspspace.Actions.paste();
        ...
        if (evt.keyCode === 46) {
          // delete
          ocspspace.Actions['delete'](ocspspace.CanvasUtils.getSelection());

          evt.preventDefault();
        } else if (evt.keyCode === 27) {
          // esc
          ocspspace.Actions.hideDialogs();
  ...
  var reloadFlowStatus = function () {
    return $.ajax({
      ...
      if (ocspspace.Common.isDefinedAndNotNull(response.controllerStatus)) {
        ...
        if (ocspspace.Common.isDefinedAndNotNull(controllerStatus.connectedNodes)) {
        ...
        if (ocspspace.Common.doBulletinsDiffer(currentBulletins, controllerStatus.bulletins)) {
          bulletinIcon.data('bulletins', controllerStatus.bulletins);

          // get the formatted the bulletins
          var bulletins = ocspspace.Common.getFormattedBulletins(controllerStatus.bulletins);
          ...
          if (bulletins.length === 0) {
            ...
            var newBulletins = ocspspace.Common.formatUnorderedList(bulletins);
            ...
            if (bulletinIcon.data('qtip')) {
              ...
              }, ocspspace.CanvasUtils.config.systemTooltipConfig));
            }
  ...
  var reloadProcessGroup = function (processGroupId) {
    ...
    return $.ajax({
      ...
      // set the revision
      ocspspace.Client.setRevision(processGroupResponse.revision);

      // get the controller and its contents
      var processGroup = processGroupResponse.processGroup;

      // set the group details
      ocspspace.Canvas.setGroupId(processGroup.id);
      ocspspace.Canvas.setGroupName(processGroup.name);
      ...
      if (ocspspace.Common.isDefinedAndNotNull(processGroup.parent)) {
        ocspspace.Canvas.setParentGroupId(processGroup.parent.id);
      } else {
        ocspspace.Canvas.setParentGroupId(null);
      }

      // since we're getting a new group, we want to clear it
      ocspspace.Graph.removeAll();

      // refresh the graph
      ocspspace.Graph.add(processGroup.contents, false);
  ...
  var reloadStatus = function (processGroupId) {
    ...
    return  $.Deferred(function (deferred) {
      $.ajax({
        ...
        if (ocspspace.Common.isDefinedAndNotNull(response.processGroupStatus)) {
          var processGroupStatus = response.processGroupStatus;

          // update all the stats
          ocspspace.Graph.setStatus(processGroupStatus);
        ...
        if (!ocspspace.Canvas.isClustered() || xhr.status !== 404) {
  ...
  return {
     ...
     * To load another group, update ocspspace.Canvas.setGroupId and call ocspspace.Canvas.reload.
     */
    reload: function () {
      return $.Deferred(function (deferred) {
        ...
        var processGroupXhr = reloadProcessGroup(ocspspace.Canvas.getGroupId());
        ...
        $.when(processGroupXhr, statusXhr).done(function (processGroupResult) {
          ...
          reloadStatus(ocspspace.Canvas.getGroupId()).done(function () {
    ...
    reloadStatus: function () {
      return $.Deferred(function (deferred) {
        ...
        $.when(reloadStatus(ocspspace.Canvas.getGroupId()), reloadFlowStatus()).done(function () {
          deferred.resolve();
        }).fail(function () {
          deferred.reject();
        });
    ...
    init: function () {
      ...
      console.log('ocsp-canvas init...');

      var canvasContainer = $('#canvas-container');
      ocspspace.Canvas.CANVAS_OFFSET = canvasContainer.offset().top;

      // initialize the application
      initCanvas();
      ocspspace.Canvas.View.init();
      // ocspspace.ContextMenu.init();
      // ocspspace.CanvasHeader.init();
      // ocspspace.CanvasToolbox.init();
      // ocspspace.CanvasToolbar.init();
      // ocspspace.GraphControl.init();
      // ocspspace.Search.init();
      // ocspspace.Settings.init();
      //
      // // initialize the component behaviors
      // ocspspace.Draggable.init();
      // ocspspace.Selectable.init();
      // ocspspace.Connectable.init();
      //
      // // initialize the chart
      // ocspspace.StatusHistory.init(configDetails.timeOffset);
      //
      // // initialize the birdseye
      // ocspspace.Birdseye.init();

      // initialize components
      // ocspspace.ConnectionConfiguration.init();
      // ocspspace.ProcessorConfiguration.init();
      // ocspspace.ProcessGroupConfiguration.init();
      // ocspspace.RemoteProcessGroupConfiguration.init();
      // ocspspace.RemoteProcessGroupPorts.init();
      // ocspspace.PortConfiguration.init();
      // ocspspace.SecurePortConfiguration.init();
      // ocspspace.LabelConfiguration.init();
      // ocspspace.ProcessorDetails.init();
      // ocspspace.ProcessGroupDetails.init();
      // ocspspace.PortDetails.init();
      // ocspspace.SecurePortDetails.init();
      // ocspspace.ConnectionDetails.init();
      // ocspspace.RemoteProcessGroupDetails.init();
      // ocspspace.GoTo.init();
      // ocspspace.Graph.init().done(function () {
    ...
    View: (function () {
      ...
      var updateComponentVisibility = function () {
        ...
        var isComponentVisible = function (d) {
          if (!ocspspace.Canvas.View.shouldRenderPerScale()) {
            return false;
          }
      ...
      return {
        ...
        init2: function () {
          ...
          behavior = d3.zoom()
            ...
            .on('zoom', function () {
              ...
              // refresh the canvas
              refreshed = ocspspace.Canvas.View.refresh({
                persist: false,
            ...
            .on('zoomend', function () {
              ...
              if (ocspspace.Common.isDefinedAndNotNull(refreshed)) {
                ...
                ocspspace.CanvasUtils.persistUserView();
        ...
        shouldRenderPerScale: function () {
          return ocspspace.Canvas.View.scale() >= MIN_SCALE_TO_RENDER;
        },
        ...
        translate: function (translate) {
          if (ocspspace.Common.isUndefined(translate)) {
        ...
        scale: function (scale) {
          if (ocspspace.Common.isUndefined(scale)) {
        ...
        fit: function () {
          ...
          var graphTop = (graphBox.top - ocspspace.Canvas.CANVAS_OFFSET) / scale;
        ...
        actualSize: function () {
          ...
          if (!selection.empty()) {
            ...
            box = {
              ...
              y: ((selectionBox.top - ocspspace.Canvas.CANVAS_OFFSET) / scale) - (translate[1] / scale),
        ...
        refresh: function (options) {
          return $.Deferred(function (deferred) {
            ...
            if (ocspspace.Common.isDefinedAndNotNull(options)) {
              persist = ocspspace.Common.isDefinedAndNotNull(options.persist) ? options.persist : persist;
              transition = ocspspace.Common.isDefinedAndNotNull(options.transition) ? options.transition : transition;
              refreshComponents = ocspspace.Common.isDefinedAndNotNull(options.refreshComponents) ? options.refreshComponents : refreshComponents;
              refreshBirdseye = ocspspace.Common.isDefinedAndNotNull(options.refreshBirdseye) ? options.refreshBirdseye : refreshBirdseye;
            }
