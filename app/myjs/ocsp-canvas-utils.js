ocspspace.CanvasUtils = (function () {
  ...
  return {
    ...
    supportsModification: function (selection) {
      ...
      if (ocspspace.CanvasUtils.isProcessor(selection) || ocspspace.CanvasUtils.isInputPort(selection) || ocspspace.CanvasUtils.isOutputPort(selection)) {
        if (ocspspace.Common.isDefinedAndNotNull(selectionData.status)) {
          supportsModification = !(selectionData.component.state === 'RUNNING' || ocspspace.Common.isDefinedAndNotNull(selectionData.status.activeThreadCount) && selectionData.status.activeThreadCount > 0);
        } else {
        ...
        if (!source.empty()) {
          if (ocspspace.CanvasUtils.isRemoteProcessGroup(source) || ocspspace.CanvasUtils.isProcessGroup(source)) {
            isSourceConfigurable = true;
          } else {
            isSourceConfigurable = ocspspace.CanvasUtils.supportsModification(source);
          }
        }
        ...
        if (!destination.empty()) {
          if (ocspspace.CanvasUtils.isRemoteProcessGroup(destination) || ocspspace.CanvasUtils.isProcessGroup(destination)) {
            isDestinationConfigurable = true;
          } else {
            isDestinationConfigurable = ocspspace.CanvasUtils.supportsModification(destination);
          }
        }
    ...
    isCopyable: function (selection) {
      ...
      var copyable = selection.filter(function (d) {
        ...
        if (ocspspace.CanvasUtils.isConnection(selected)) {
          ...
          return ocspspace.CanvasUtils.isProcessor(selected) || ocspspace.CanvasUtils.isFunnel(selected) || ocspspace.CanvasUtils.isLabel(selected) || ocspspace.CanvasUtils.isProcessGroup(selected) || ocspspace.CanvasUtils.isRemoteProcessGroup(selected) || ocspspace.CanvasUtils.isInputPort(selected) || ocspspace.CanvasUtils.isOutputPort(selected);
        }
      });
