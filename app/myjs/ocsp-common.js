ocspspace.Common = {
  ...
  setAuthorities: function (roles) {
    ocspspace.Common.authorities = roles;
  },
  ...
  isDFM: function () {
    var dfm = false;
    if (ocspspace.Common.isDefinedAndNotNull(ocspspace.Common.authorities)) {
      $.each(ocspspace.Common.authorities, function (i, authority) {
  ...
  handleAjaxError: function (xhr, status, error) {
    ...
    if (xhr.status === 400 || xhr.status === 404 || xhr.status === 409) {
      ...
      if (xhr.status < 99 || xhr.status === 12007 || xhr.status === 12029) {
        ...
        if (ocspspace.Common.isDefinedAndNotNull(status)) {
          if (status === 'timeout') {
  ...
  closeCanvas: function () {
    ...
    if (ocspspace.Common.isDefinedAndNotNull(ocspspace.Canvas)) {
      ...
      ocspspace.Canvas.stopRevisionPolling();
      ocspspace.Canvas.stopStatusPolling();
    }
  },
  ...
  populateField: function (target, value) {
    if (ocspspace.Common.isUndefined(value) || ocspspace.Common.isNull(value)) {
  ...
  formatPropertyTooltip: function (propertyDescriptor, propertyHistory) {
    ...
    if (ocspspace.Common.isDefinedAndNotNull(propertyDescriptor)) {
      if (!ocspspace.Common.isBlank(propertyDescriptor.description)) {
        tipContent.push(ocspspace.Common.escapeHtml(propertyDescriptor.description));
      }
      if (!ocspspace.Common.isBlank(propertyDescriptor.defaultValue)) {
        tipContent.push('<b>Default value:</b> ' + ocspspace.Common.escapeHtml(propertyDescriptor.defaultValue));
      }
      if (!ocspspace.Common.isBlank(propertyDescriptor.supportsEl)) {
        tipContent.push('<b>Supports expression language:</b> ' + ocspspace.Common.escapeHtml(propertyDescriptor.supportsEl));
      }
    }

    if (ocspspace.Common.isDefinedAndNotNull(propertyHistory)) {
      if (!ocspspace.Common.isEmpty(propertyHistory.previousValues)) {
        var history = [];
        $.each(propertyHistory.previousValues, function (_, previousValue) {
          history.push('<li>' + ocspspace.Common.escapeHtml(previousValue.previousValue) + ' - ' + ocspspace.Common.escapeHtml(previousValue.timestamp) + ' (' + ocspspace.Common.escapeHtml(previousValue.userName) + ')</li>');
        });
  ...
  formatProperty: function (name, value) {
    return '<div><span class="label">' + ocspspace.Common.formatValue(name) + ': </span>' + ocspspace.Common.formatValue(value) + '</div>';
  },
  ...
  formatValue: function (value) {
    if (ocspspace.Common.isDefinedAndNotNull(value)) {
      if (value === '') {
        return '<span class="blank">Empty string set</span>';
      } else {
        return ocspspace.Common.escapeHtml(value);
      }
    } else {
  ...
  escapeHtml: (function () {
    ...
    return function (string) {
      if (ocspspace.Common.isDefinedAndNotNull(string)) {
  ...
  submit: function (method, url, params) {
    ...
    if (ocspspace.Common.isDefinedAndNotNull(params)) {
  ...
  formatDuration: function (duration) {
    ...
    var days = duration / ocspspace.Common.MILLIS_PER_DAY;
    days = days >= 1 ? parseInt(days, 10) : 0;
    duration %= ocspspace.Common.MILLIS_PER_DAY;

    // remaining duration should be less than 1 day, get number of hours
    var hours = duration / ocspspace.Common.MILLIS_PER_HOUR;
    hours = hours >= 1 ? parseInt(hours, 10) : 0;
    duration %= ocspspace.Common.MILLIS_PER_HOUR;

    // remaining duration should be less than 1 hour, get number of minutes
    var minutes = duration / ocspspace.Common.MILLIS_PER_MINUTE;
    minutes = minutes >= 1 ? parseInt(minutes, 10) : 0;
    duration %= ocspspace.Common.MILLIS_PER_MINUTE;

    // remaining duration should be less than 1 minute, get number of seconds
    var seconds = duration / ocspspace.Common.MILLIS_PER_SECOND;
    seconds = seconds >= 1 ? parseInt(seconds, 10) : 0;

    // remaining duration is the number millis (don't support sub millisecond resolution)
    duration = Math.floor(duration % ocspspace.Common.MILLIS_PER_SECOND);

    // format the time
    var time = ocspspace.Common.pad(hours, 2, '0') +
      ':' +
      ocspspace.Common.pad(minutes, 2, '0') +
      ':' +
      ocspspace.Common.pad(seconds, 2, '0') +
      '.' +
      ocspspace.Common.pad(duration, 3, '0');
  ...
  formatDataSize: function (dataSize) {
    // check terabytes
    var dataSizeToFormat = parseFloat(dataSize / ocspspace.Common.BYTES_IN_TERABYTE);
    if (dataSizeToFormat > 1) {
      return dataSizeToFormat.toFixed(2) + " TB";
    }

    // check gigabytes
    dataSizeToFormat = parseFloat(dataSize / ocspspace.Common.BYTES_IN_GIGABYTE);
    if (dataSizeToFormat > 1) {
      return dataSizeToFormat.toFixed(2) + " GB";
    }

    // check megabytes
    dataSizeToFormat = parseFloat(dataSize / ocspspace.Common.BYTES_IN_MEGABYTE);
    if (dataSizeToFormat > 1) {
      return dataSizeToFormat.toFixed(2) + " MB";
    }

    // check kilobytes
    dataSizeToFormat = parseFloat(dataSize / ocspspace.Common.BYTES_IN_KILOBYTE);
    if (dataSizeToFormat > 1) {
  ...
  formatFloat: function (f) {
    if (ocspspace.Common.isUndefinedOrNull(f)) {
      return 0.00 + '';
    }
    return f.toFixed(2) + '';
  },
  ...
  formatDateTime: function (date) {
    return ocspspace.Common.pad(date.getMonth() + 1, 2, '0') +
      '/' +
      ocspspace.Common.pad(date.getDate(), 2, '0') +
      '/' +
      ocspspace.Common.pad(date.getFullYear(), 2, '0') +
      ' ' +
      ocspspace.Common.pad(date.getHours(), 2, '0') +
      ':' +
      ocspspace.Common.pad(date.getMinutes(), 2, '0') +
      ':' +
      ocspspace.Common.pad(date.getSeconds(), 2, '0') +
      '.' +
      ocspspace.Common.pad(date.getMilliseconds(), 3, '0');
  },
  ...
  parseDateTime: function (rawDateTime) {
    ...
    if (!ocspspace.Common.isDefinedAndNotNull(rawDateTime)) {
      return new Date();
    }
  ...
  isDefinedAndNotNull: function (obj) {
    return !ocspspace.Common.isUndefined(obj) && !ocspspace.Common.isNull(obj);
  },
  ...
  isBlank: function (str) {
    return ocspspace.Common.isUndefined(str) || ocspspace.Common.isNull(str) || $.trim(str) === '';
  },
  ...
  getFormattedBulletins: function (bulletins) {
    ...
    $.each(bulletins, function (j, bulletin) {
      ...
      var nodeAddress = '';
      if (ocspspace.Common.isDefinedAndNotNull(bulletin.nodeAddress)) {
        nodeAddress = '-&nbsp' + ocspspace.Common.escapeHtml(bulletin.nodeAddress) + '&nbsp;-&nbsp;';
      }
      ...
      var formattedBulletin = $('<div>' +
        ocspspace.Common.escapeHtml(bulletin.timestamp) + '&nbsp;' +
        nodeAddress + '&nbsp;' +
        '<b>' + ocspspace.Common.escapeHtml(bulletin.level) + '</b>&nbsp;' +
